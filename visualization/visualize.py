...
class Visualizer:
    def __init__(self, temp_context, vis_color_pred, evaluation_qualitative_res_path, evaluation_visualization_set, vis_rgb, vis_pred_depth, vis_gt_depth, min_depth, max_depth, vis_unc=True):
    ...
    def visualize_unc(self, unc_maps, filename, weather):
        for i, unc_map in enumerate(unc_maps):
            unc_map = np.squeeze(unc_map.cpu().numpy())
            unc_map_normalized = (unc_map - np.min(unc_map)) / (np.max(unc_map) - np.min(unc_map))
            
            unc_map_colored = plt.cm.plasma(unc_map_normalized)  # Get the plasma colormap
            unc_map_colored = (unc_map_colored[:, :, :3] * 255).astype(np.uint8)  # Convert to uint8
        
            cv2.imwrite(os.path.join(self.evaluation_qualitative_res_path, f"{filename}_unc_{weather}_{i}.jpg"), unc_map_colored)
        ...
        for j, (img, inv_depth, depth_gt_) in enumerate(zip(imgs, inv_depths, depth_gt)):
            ...
            if not self.evaluation_visualization_set or filename in self.evaluation_visualization_set:
                ...
                if self.vis_unc:
                    ...
                    unc_maps = outputs['unc_maps']
                    self.visualize_unc(unc_maps, filename, batch['weather'][0])
