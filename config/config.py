...
_C.TRAINING = CN()
_C.TRAINING.SELF_DISTILL = False
...
_C.LOAD.DAYTIME_TRANSLATION_TEACHER_PATHS = None  # Path where the teacher model (e.g. a baseline ckpt) is stored
_C.LOAD.INIT_CHECKPOINT_PATH = None  # Checkpoint to initialize depth (and pose) model weights, e.g. md4allDD (decoupled from the pseudo-label teacher)
...
_C.DATASET = CN()
_C.DATASET.DATAROOT = '../nuscenes'  # Path to main data folder containing nuScenes or RobotCar
...
_C.DATASET.WEATHER.TRAIN = ['day-clear', 'day-rain', 'night-clear', 'night-rain']  # Weather conditions allowed in training set
...
_C.DATASET.LIDAR_PRJ_SIZE = CN()
_C.DATASET.LIDAR_PRJ_SIZE.HEIGHT = 900  # Height for depth gt
_C.DATASET.LIDAR_PRJ_SIZE.WIDTH = 1600  # Width for depth gt
...
_C.LOSS.SUPERVISED.TEACHER_NUM = 2  # Number of weather-expert teachers: 1 = single-teacher expert training, >=2 = UAMTD multi-teacher
...
_C.EVALUATION.UNC = CN()
_C.EVALUATION.UNC.VIS_UNC=False
