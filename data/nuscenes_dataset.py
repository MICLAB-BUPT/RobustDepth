...
from utils.str_tensor_utils import StringTensorUtils
# import clip
...
class NuScenesDataset:
    def __init__(self, cfg, transform, dataset_version):
        ...
        self.lidar_prj_height = cfg.DATASET.LIDAR_PRJ_SIZE.HEIGHT
        self.lidar_prj_width = cfg.DATASET.LIDAR_PRJ_SIZE.WIDTH
        print("orig h:{} w:{}".format(self.original_height, self.original_width))
        print("lidar h:{} w:{}".format(self.lidar_prj_height, self.lidar_prj_width))
        ...
        print("train conditions:{}".format(self.weather_conditions_train))
        ...
        # Load the text description from json file
        """
        Load the JSON file and return a dictionary with filenames as keys and text as values.
        """
        # json_path = os.path.join(self.nusc.dataroot, 'text_llava_onevision.json')
        # self.text_dict = None
        ...
        #         text_dict = {item["filename"]: item["text"] for item in data}
        #     except Exception as e:
        #         print(f"Error loading JSON file {json_path}: {e}")
        #     self.text_dict = text_dict
        #     self.console_logger.info(f"[Info] Finished read caption {dataset_version} official database!")
               
        # self.str_tensor_utils = StringTensorUtils(max_length=1000)
    ...
    def getitem(self, index, mode):
        ...
        for temp in temp_context:
            # if mode == 'train':
            img, filepath = self.get_color_npy(frame_idx, camera_sensor, temp)
            # else:
            #     img, filepath = self.get_color_jpg(frame_idx, camera_sensor, temp)
        ...
        if mode!='train' and self.check_depth(frame_idx, camera_sensor):
    ...
    def get_text(self, filename):
        if self.text_dict is not None:
            return self.text_dict[filename]
        else:
            return None

    def get_img_filename(self, index, mode='train'):
        ...
        filename = self.get_cam_sample_data(frame_idx, camera_sensor, 0)['filename']
    ...
    def get_color_jpg(self, frame_index, camera_sensor, temp_shift):
        filename = self.get_cam_sample_data(frame_index, camera_sensor, temp_shift)['filename']
        return Image.open(os.path.join(self.nusc.dataroot, filename)), filename

    def get_color_npy(self, frame_index, camera_sensor, temp_shift):
        filename = self.get_cam_sample_data(frame_index, camera_sensor, temp_shift)['filename']
        npy_filename = filename.replace('.jpg', '.npy')
        np_array = np.load(os.path.join(self.nusc.dataroot, npy_filename))
    ...
    def get_depth(self, index, sensor):
        ...
        points, depth, _ = self.nusc_explorer.map_pointcloud_to_image(pointsensor_token=lidar_data['token'],
                                                                      camera_token=cam_data['token'])
        lidar_proj = self.generate_image_from_points(points[:2], depth, (self.lidar_prj_height, self.lidar_prj_width))
        return np.expand_dims(lidar_proj, axis=0)
    ...
    def __len__(self, mode):
        if mode == 'train':
            return len(self.train_idcs)
        elif mode == 'val':
            return len(self.val_idcs)
        elif self.mode == 'eval':
            return len(self.test_idcs)
        else:
