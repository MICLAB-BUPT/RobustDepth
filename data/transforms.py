...
class Resize:
    ...
    def __call__(self, sample):
        ...
        # print(sample[(self.sample_keys[0], 0)].size)
        # print(self.size)
        if sample[(self.sample_keys[0], 0)].size[0] != self.size[1]:
            # print(np.array(sample[(self.sample_keys[0], 0)]))
    ...
    #         for i, sample_key in enumerate(self.sample_keys):
    #             if (sample_key, frame_idx) in sample:
    #                 if i == 0:
    #                     resized_img = self.resize(sample[(sample_key, frame_idx)])
    #                     out_img = resized_img
    #                 else:
    #                     out_img = resized_img.copy()
    #                 sample[(sample_key, frame_idx)] = out_img
            ...
            if self.direction == 'night->day' and 'night' in sample['weather'] or \
                ...
                for sample_key in self.sample_keys:
                    for temp in self.temp_context:
                        ...
                        filename = os.path.join(self.path, sample[('filename', temp)])
                        npy_filename = filename.replace('.jpg', '.npy')
                        np_array = np.load(npy_filename)
                        pil_image = Image.fromarray(np_array)
                        sample[(sample_key, temp)] = pil_image
