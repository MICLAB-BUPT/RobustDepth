...
class Md4All(pl.LightningModule):
    ...
    def __init__(self, cfg, is_train):
        ...
        print("device at:{}".format(self.depth_model.device))
        # Decoupled initialization: prefer an explicit INIT_CHECKPOINT_PATH (e.g. md4allDD) for warm-starting
        # the depth/pose weights; fall back to the self-distillation teacher path for backward compatibility.
        init_path = cfg.LOAD.INIT_CHECKPOINT_PATH
        if init_path is None and cfg.TRAINING.SELF_DISTILL:
            init_path = cfg.LOAD.DAYTIME_TRANSLATION_TEACHER_PATH
        if init_path is not None:
            print(f"Initializing depth/pose models from: {init_path}")
            init_sd = torch.load(init_path, map_location='cpu')['state_dict']
            self.depth_model.load_state_dict(
                {k[len("depth_model."):]: w for k, w in init_sd.items() if k.startswith("depth_model.")},
                strict=False)
            self.pose_model.load_state_dict(
                {k[len("pose_model."):]: w for k, w in init_sd.items() if k.startswith("pose_model.")},
                strict=False)
        if cfg.TRAINING.SELF_DISTILL:
            # Distillation setting: the pose network is frozen (only the depth student is optimized)
    ...
    def depth_estimation(self, inputs, is_train):
        ...
        outputs, encoder_feats = self.depth_model(cond_flip(inputs[('color_aug', 0)]), inputs['weather_depth'])
        inv_depth = {scale: cond_flip(disp) for scale, disp in outputs.items()}
        return inv_depth, encoder_feats
    ...
    def shared_step(self, inputs, is_train):
        disps, encoder_feats = self.depth_estimation(inputs, is_train)
    ...
    def configure_optimizers(self):
        optimizer = optim.Adam([
            ...
            {'params': self.loss.supervised_loss.unc_miner.parameters(), 'lr': self.depth_lr}
        ],
