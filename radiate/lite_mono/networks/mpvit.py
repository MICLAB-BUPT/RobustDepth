...
def mpvit_xsmall(**kwargs):
    ...
    # logger = get_root_logger()
    load_state_dict(model, checkpoint, strict=False, logger=None)
    del checkpoint
    # del logger
    model.default_cfg = _cfg_mpvit()
    return model
...
def mpvit_small(pretrained_path=None, **kwargs):
    """mpvit_small :
    ...
    if pretrained_path is not None:
        checkpoint = torch.load(pretrained_path, map_location=lambda storage, loc: storage)['model']
    # logger = get_root_logger()
        load_state_dict(model, checkpoint, strict=False, logger=None)
        del checkpoint
    # del logger
